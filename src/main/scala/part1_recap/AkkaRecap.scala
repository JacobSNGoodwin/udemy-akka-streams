...
object AkkaRecap extends App {
  class SimpleActor extends Actor with ActorLogging with Stash {
    override def receive: Receive = {
      case "createChild" =>
        ...
        childActor ! "hello"
      // stash messages that will be appropriate for new handler
      case "stashThis" =>
        stash()
    ...
    // determines how to handle exceptions in children
  ...
  import system.dispatcher // implements execution context interface
  system.scheduler.scheduleOnce(2 seconds) {
    actor ! "delayed happy birthday"
  }

  // Akka patterNs including FSM (Finite State Machines) + ask pattern
  import akka.pattern.ask
  implicit val timeout = Timeout(3 seconds)
  val future = actor ? "question" // responds with first response... times out after 3 seconds

  // pipe pattern
  import akka.pattern.pipe
  val anotherActor = system.actorOf(Props[SimpleActor], "anotherSimpleActor")
  future.mapTo[String].pipeTo(anotherActor) // when future completed, value sent to another actor as a message
}
