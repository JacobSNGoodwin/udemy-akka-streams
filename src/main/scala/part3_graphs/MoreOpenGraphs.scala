...
//  max3RunnableGraph.run()
  ...
  // Uniform means inputs all of the same type
  ...
  case class Transaction(id: String, source: String, recipient: String, amount: Double, data: Date)

  val transactionSource = Source(List(
    Transaction("9845794", "Paul", "Jim", 100.00, new Date),
    Transaction("3897343", "Daniel", "Jim", 100254.25, new Date),
    Transaction("8974652", "Jim", "Alice", 7000, new Date)
  ))

  val bankProcessor = Sink.foreach[Transaction](println)
  val suspiciousAnalysisService = Sink.foreach[String](txnId => println(s"Suspeicious transaction ID: $txnId"))
  ...
  val suspiciousTxnStaticGraph = GraphDSL.create() { implicit builder =>
    ...
    // step 4 - FanOutShape with two output types
    // note that we have to use new keyword for this shape
