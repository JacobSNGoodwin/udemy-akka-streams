...
object OpenGraphs extends App {
  ...
  val secondSource = Source(42 to 2000)
...
//  firstSource.to(sinkGraph).run()
  ...
  /*
    Exercise: possible to create a slow from a sink and a source?
   */
  ...
  def fromSinkAndSource[A, B](sink: Sink[A, _], source: Source[B, _]): Flow[A, B, _] =
    Flow.fromGraph(
      GraphDSL.create() { implicit builder =>
        ...
        // step 4 - distinct and unconnected components
        FlowShape(sinkShape.in, sourceShape.out)
      }
    )

  // coupled allows ending back pressure signals
